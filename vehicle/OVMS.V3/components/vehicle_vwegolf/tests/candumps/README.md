...
(gear 0x187, VIN 0x6B4, SoC 0x131, pack I/V 0x191, speed 0x0FD, etc.) onto KCAN, so
a single KCAN tap sees everything — verified in drive captures (every decoded id,
including 0x187 and 0x6B4, appears on `3R11` with valid content). `IncomingFrameCan3`
is the sole decode path. FCAN (can2) is not registered: it carried nothing KCAN
doesn't already have, so tapping it only added controller RX/ISR load for no data.
